...
namespace Services
{
    ...
    {
        private readonly LinkGenerator _linkGenerator; //linklenen datalar 
        private readonly IDataShaper<BookDto> _dataShaper; //şekillenen datalar
        ...
        public LinkResponse TryGenerateLinks(IEnumerable<BookDto> booksDto,
            string fields,
            HttpContext httpContext) //link oluşturmaya çalışıyoruz
        {
            var shapedBooks = ShapeData(booksDto, fields);
            if (ShouldGenerateLinks(httpContext))
                return ReturnLinkedBooks(booksDto, fields, httpContext, shapedBooks);//oluşturabildiysek oluşturduğumuz listeyi dönüyoruz.
            return ReturnShapedBooks(shapedBooks); //oluşturadıysak şekillendirdiğimiz listeyi dönüyoruz. 
        }
        ...
        private bool ShouldGenerateLinks(HttpContext httpContext) //link oluşrurabiliyor musun, oluştura mıyormusun
        {
